...
double gbyzantine_fraction;
int pool_size;
char gobjfunc[20];

/* Mutexes */
pthread_mutex_t *vmutex;	//Array of mutexes
...
//	cout << "Time = " << time(0);
	...
	gbasic_seed = (double) (time(NULL) % 100)/ (double) (time(NULL) % 100 + 1);
	printf("\ngbasic_seed =  %lf\n", gbasic_seed);
...
//cout << x_lower[k] << " " << x_upper[k] << endl;
	...
	byzthreshold = gnum_threads*gbyzantine_fraction;
...
//jcross = rnd(1, (lchrom + MAX_LIFECYCLE));
    ...
    {
            if(jcross >= (int)(k*UINTSIZE))
            {
                child1[k-1] = parent1[k-1];
                child2[k-1] = parent2[k-1];
            }
            else if((jcross < (int)(k*UINTSIZE)) && (jcross > (int)((k-1)*UINTSIZE)))
            {
                mask = 1;
                for(j = 1; j <= (int)(jcross-1-((k-1)*UINTSIZE)); j++)
                {
                    temp = 1;
                    mask = mask<<1;
                    mask = mask|temp;
...
void* GAThread::ThreadFunction(void* p)
{
	...
	for(pg->gen_no = 1; pg->gen_no<= pg->max_gen; pg->gen_no++)
	{
		...
		if(pg->flip(pg->failure_prob))
		{
			...
			gnum_failed++;
			cerr << "Thread " << pg->id << " failed in generation " << pg->gen_no << '\n';
...
#ifdef BARRIER_SYNC
//		cout << "Waiting on barrier Thread " << pg->id << " gen no " << pg->gen_no << '\n';
...
//#else
		...
		interval.tv_sec = 0; interval.tv_nsec = 500000;
...
void initpool()
{
	...
	cout << "Initial Pool " << pool_size << endl;
...
int main(int argc, char* argv[])
{
	...
	if(argc != 4)
	{
		cout << "\nUsage: ./pool objfuncname NUM_THREADS BYZANTINEFRACTION < inputfile\n\n";
    cout << "look at the objective() function inside pool.cpp for available\n"
             "objective functions. The benchmark function can be passed using f1 f2 f3 f4\n\n";
    cout << "specify a dummy byzantine fraction even if not doing a "
             "byzantine simulation\n"
             "A simulation is a byzantine simulation if "
             "#define BYZANTINE_ANTIELITISM is uncommented in defines.h\n";
    cout << "\n Example: ./pool f4 16 0.00 < ../inputs/inputf4\n\n";
		return -1;
	}
	else
	{
    strcpy(gobjfunc, argv[1]);
		gnum_threads = atoi(argv[2]);
		gbyzantine_fraction = atof(argv[3]);
		cout << "Number of Threads = " << gnum_threads << '\n';
		cout << "ByzPercentage=" << gbyzantine_fraction << '\n';
	}
	gnum_failed = 0;
	pthread_attr_t attr;
	...
	// Barrier initialization
    	if(pthread_barrier_init(&barr, NULL, gnum_threads))
    	{
...
//return -100;
...
#ifdef POOL_SNAPSHOT
	cout << "Final Pool " << pool_size << endl;
	//find minima maxima and poollength
	...
	if(pthread_barrier_destroy(&barr))
		cerr << "Could not destroy barrier\n";
#endif
	//fclose(fp_out);
	free_pool();
	delete [] pool;
	...
	delete [] vmutex;
	cout << '\n' << " O.K Good bye !!!" << '\n';
	//so that gbasic_seed gets set to a different value
	//sleep(1);
...
double objectivecopy(double *x)
{
  ...
  //-10 < xi < 10
  //i going from 0 to 4
  if (strcmp(gobjfunc,"f1") == 0)
  {
    fit = 0;
    double ten = 1.0;
    for(int i = 0; i < gnum_var; i++)
    {
      fit += ten*x[i]*x[i];
      ten *= 10.0;
    }
  ...
  cout << "oh oh shouldnt have reached here, returning 0 \n";
  return 0;
}
