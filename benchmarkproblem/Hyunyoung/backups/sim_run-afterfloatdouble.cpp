...
/* 17th October 2008
   @gautam Dont forget to define 
    MINMAX = 1 for minimization or -1 for Maximization
*/
...
//#define MINMAX 1	//1 for minimization and -1 for maximization
...
int execute(void) {
	...
	for (t=1; t <= VARY_NUM_THREADS; ++t) { /* construct file name for output */
		...
		for (run_num=1; run_num <= RUN; run_num++) { /* inner for each run */
			...
			strcat(ofname, ".rslt");
			printf("%s\n", ofname);
			sprintf(cmd_str, "./pool %d < inputf3 > %s", i, ofname);
			//printf("%s\n", cmd_str);
			system(cmd_str);
...
int gather_data(void) {
	...
	int  z, g, i, j,  t, val_num, gen_num, byz_threshold;
