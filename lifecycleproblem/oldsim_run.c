...
int gather_data(void)
{
    ...
    double each_gen_val_sum[MAX_GEN] = {0};
		...
		float  mono_best_vals[VARY_NUM_THREADS][MAX_GEN] = {0};
	...
	{  //inner for multiple runs
			...
			if (mono_best_vals[t][g] < bval)
                             mono_best_vals[t][g] = bval;
                          if (mono_best_vals[t][g-1] > mono_best_vals[t][g])
	...
	for (g = 1; g <= max_gen; ++g) 
		each_gen_best_avg[t][g] = each_gen_val_sum[g] / num_threads;
